...
if(isset($_POST['replyBtn'])) {
    ...
    if (empty($reply)) {
        ...
        $update = "UPDATE feedback SET reply='$reply' WHERE fb_id='" . $_GET['get'] . "'";
        if ($query = mysqli_query($con, $update)) {
            $swal = "success";
            $_SESSION['action'] = "Reply sent successfully";
            header('location:new_cust_feedback.php');
                ...
                <h2 class="legend">Manage customer account</h2>
                        <fieldset class="">
                                ...
                                <table id="dataTables-example"class="table table-striped table-responsive tableData">
                                   ...
                                   <?php
                                   $select="SELECT * FROM feedback f INNER JOIN  customer c ON f.cust_id = c.cust_id WHERE fb_id='".$_GET['get']."' ";
